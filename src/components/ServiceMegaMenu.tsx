...
export default function ServiceMegaMenu({ onClose }: Props) {
  ...
  return (
    <div className="w-full bg-white border-t-2 border-[#1a3a8f]" style={{ boxShadow: "0 24px 60px rgba(0,0,0,0.18)" }}>
      <div className="mx-auto w-full max-w-[1600px] px-5">
        <div className="flex items-center gap-0 py-3 border-b border-gray-100 overflow-x-auto">
          ...
          <div className="ml-auto pl-6 text-xs text-gray-400 hidden xl:flex items-center gap-1.5 whitespace-nowrap"><MapPin size={11} /><span className="font-semibold text-[#1a3a8f]">{isYashobhoomi ? "Yashobhoomi" : activeVenue?.city}</span>{activeVenue && <><span>/</span><span>{activeVenue.name.split(",")[0]}</span></>}</div>
        </div>
        ...
        {!loading && !error && (
          ...
          >
            ...
            {!isYashobhoomi && <Panel title={`Venues ${activeVenue?.city ? `in ${activeVenue.city}` : ""}`} icon={<Building2 size={9} className="inline mr-1" />}>
              {venues.map((venue) => (
                <button key={venue.id} onClick={() => setActiveVenueId(venue.subVenueId)} className={`w-full text-left flex items-start justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all mb-0.5 gap-2 ${activeVenue?.id === venue.id ? "bg-blue-50 text-[#1a3a8f] border border-blue-200" : "text-gray-600 hover:bg-gray-50 hover:text-[#1a3a8f]"}`}>
                  <span className="leading-snug">{venue.name}</span>
                  <ChevronRight size={12} className="flex-shrink-0 opacity-60 mt-0.5" />
                </button>
              ))}
              {activeVenue && (
                <Link href={`/venue/${activeVenue.locationId}/${activeVenue.subVenueId}`} onClick={onClose} className="mt-4 flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-3 text-sm font-semibold text-gray-700 hover:border-[#1a3a8f] hover:bg-blue-50 hover:text-[#1a3a8f]">
            ...
            <Panel title="Services Available">
              {catalog.services.map((service) => (
                <button key={service.id} onMouseEnter={() => setActiveServiceId(service.id)} onClick={() => setActiveServiceId(service.id)} className={`w-full text-left flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all mb-0.5 ${activeService?.id === service.id ? "bg-yellow-50 text-gray-900 border border-yellow-300 shadow-sm" : "text-gray-700 hover:bg-gray-50 hover:text-[#1a3a8f]"}`}>
            ...
            <div className="bg-gradient-to-b from-blue-50 to-white px-4 py-6">
              ...
              {selectedLocationName ? <p className="mb-3 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-[#1a3a8f]">Location: {selectedLocationName}</p> : null}
              <Link href={requirementHref} onClick={onClose} className="block w-full text-center bg-[#1a3a8f] text-white px-3 py-2.5 rounded-lg text-sm font-semibold hover:bg-[#152e75] transition-colors mb-2.5">Book Now</Link>
              <Link href={requirementHref} onClick={onClose} className="block w-full text-center border border-[#1a3a8f] text-[#1a3a8f] px-3 py-2.5 rounded-lg text-sm font-semibold hover:bg-blue-50 transition-colors">Get Quote</Link>
            </div>
          </div>
        )}
      </div>
