...
function ServicesSection({ title, description, services }: { title: string; description: string; services: CatalogService[] }) {
  return <section className="bg-gray-50 py-16 lg:py-20"><div className="max-w-[1600px] mx-auto px-8"><SectionTitle title={title} description={description} /><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">{services.map((service) => <Link key={service.id} href={service.packages[0]?.href ?? `/service/${service.id}`} className="group bg-white rounded-2xl border border-gray-100 p-6 hover:border-[#1a3a8f] hover:shadow-lg transition-all duration-200"><h3 className="font-semibold text-gray-900 mb-2 group-hover:text-[#1a3a8f] transition-colors">{service.label}</h3><p className="text-sm text-gray-500 leading-relaxed mb-3 line-clamp-3">{service.description || `${service.packages.length} package${service.packages.length === 1 ? "" : "s"} available`}</p><div className="flex items-center gap-1 text-[#1a3a8f] text-sm font-medium">Learn more <ArrowRight size={14} /></div></Link>)}</div></div></section>;
}
