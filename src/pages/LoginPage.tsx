...
import { loginSchema, phoneOtpRequestSchema, phoneOtpVerifySchema, type LoginValues } from "@/lib/validators";

export default function LoginPage() {
  const { user, loading, login, loginWithOtp, requestPhoneOtp } = useAuth();
  ...
  const handleOtpRequest = async () => {
    setFieldErrors({});
    setSubmitError("");
    const result = phoneOtpRequestSchema.safeParse({ phone: otpForm.phone });
    if (!result.success) {
    ...
    setStatus("loading");
    try {
      const response = await requestPhoneOtp(result.data.phone);
      setOtpSent(true);
      setDevOtp(response.devOtp || "");
      setSubmitError(response.message);
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : "Unable to send OTP.");
    } finally {
      setStatus("idle");
    }
  };
  ...
  const handleOtpLogin = async (e: React.FormEvent) => {
    ...
    setFieldErrors({});
    setSubmitError("");
    const result = phoneOtpVerifySchema.safeParse(otpForm);
    if (!result.success) {
      setFieldErrors(Object.fromEntries(result.error.errors.map((error) => [error.path[0] as string, error.message])));
      return;
    }
    setStatus("loading");
    try {
      await loginWithOtp(result.data);
      setLocation(redirectTo);
    } catch (error) {
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <div className="px-8 py-8">
            <div className="mb-5 grid grid-cols-2 rounded-xl bg-gray-100 p-1 text-sm font-semibold">
              <button type="button" onClick={() => setMode("email")} className={`rounded-lg py-2 ${mode === "email" ? "bg-white text-[#1a3a8f] shadow-sm" : "text-gray-500"}`}>Email</button>
              <button type="button" onClick={() => setMode("phone")} className={`rounded-lg py-2 ${mode === "phone" ? "bg-white text-[#1a3a8f] shadow-sm" : "text-gray-500"}`}>Phone OTP</button>
            </div>
            {mode === "email" ? <form onSubmit={handleSubmit} className="space-y-5">
              <div>
              ...
              <form onSubmit={handleOtpLogin} className="space-y-5">
                <div>
                  ...
                  <input type="tel" required value={otpForm.phone} onChange={(e) => setOtpForm({ ...otpForm, phone: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a8f]" placeholder="+91 98100 97323" />
                ...
                {otpSent ? (
                  <div>
                    ...
                    <input required maxLength={6} value={otpForm.code} onChange={(e) => setOtpForm({ ...otpForm, code: e.target.value.replace(/\D/g, "") })} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a8f]" placeholder="6 digit OTP" />
                    {fieldErrors.code ? <p className="mt-2 text-sm text-red-600">{fieldErrors.code}</p> : null}
                  </div>
                ) : null}
                {submitError ? <p className={`text-sm ${otpSent ? "text-green-700" : "text-red-600"}`}>{submitError}</p> : null}
                {devOtp ? <p className="rounded-lg border border-yellow-200 bg-yellow-50 px-3 py-2 text-sm font-bold text-yellow-800">Development OTP: {devOtp}</p> : null}
                <button type={otpSent ? "submit" : "button"} onClick={otpSent ? undefined : handleOtpRequest} disabled={status === "loading"} className="w-full flex items-center justify-center gap-2 bg-[#1a3a8f] text-white py-3 rounded-xl font-semibold hover:bg-[#152e75] disabled:opacity-60">
                  <LogIn size={18} />
                  {status === "loading" ? "Please wait..." : otpSent ? "Verify OTP & Sign In" : "Send OTP"}
                </button>
              </form>
            )}
