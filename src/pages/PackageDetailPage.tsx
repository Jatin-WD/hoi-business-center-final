...
export default function PackageDetailPage({ params }: Props) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href="/service" className="font-medium text-[#1a3a8f] hover:underline">Back to Services</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-[#0f2460] to-[#1a3a8f] px-8 py-14 text-white">
        <div className="mx-auto max-w-[1600px]">
          <div className="mb-4 flex flex-wrap items-center gap-2 text-sm text-blue-200">
          ...
          <div className="mt-3 flex items-baseline gap-3">
            ...
            <span className="text-sm text-blue-200">{priceNote}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
          <div className="space-y-8 lg:col-span-2">
            <section className="rounded-2xl border border-gray-100 bg-white p-7">
              ...
              <div className="mt-4 inline-flex items-center gap-2 rounded-lg bg-blue-50 px-3 py-1.5 text-sm text-[#1a3a8f]">
              ...
              <section className="rounded-2xl border border-gray-100 bg-white p-7">
                ...
                <div className="space-y-2">
                  {otherPackages.map((pkg) => (
                    <Link key={`${pkg.category}-${pkg.subcategory}`} href={`/packages/${pkg.category}/${pkg.subcategory}${selectedLocation ? `?location=${encodeURIComponent(selectedLocation)}` : ""}`} className="group flex items-center justify-between rounded-xl border border-gray-100 px-4 py-3 transition-all hover:border-[#1a3a8f] hover:bg-blue-50">
                      <span className="text-sm font-medium text-gray-700 group-hover:text-[#1a3a8f]">{pkg.title}</span>
                      <ChevronRight size={16} className="text-gray-300 group-hover:text-[#1a3a8f]" />
                    </Link>
                  ))}
                </div>
              </section>
          ...
          <aside className="space-y-5">
            <div className="sticky top-28 rounded-2xl border border-gray-100 bg-white p-7">
              ...
              {selectedLocation ? <p className="mb-5 rounded-lg bg-blue-50 px-3 py-2 text-sm font-semibold text-[#1a3a8f]">Location: {selectedLocation}</p> : null}
              <Link href={contactHref} className="mb-3 block w-full rounded-xl bg-[#1a3a8f] py-3 text-center font-semibold text-white transition-colors hover:bg-[#152e75]">Book Now</Link>
              <Link href={contactHref} className="block w-full rounded-xl border border-[#1a3a8f] py-3 text-center font-semibold text-[#1a3a8f] transition-colors hover:bg-blue-50">Request Quote</Link>
              <div className="mt-6 space-y-3 border-t border-gray-100 pt-5">
                <a href="tel:+919810097323" className="flex items-center gap-3 text-sm text-gray-600 transition-colors hover:text-[#1a3a8f]"><Phone size={16} className="text-[#1a3a8f]" />+91 98100 97323</a>
                <a href="mailto:thlim@kilindia.in" className="flex items-center gap-3 text-sm text-gray-600 transition-colors hover:text-[#1a3a8f]"><Mail size={16} className="text-[#1a3a8f]" />thlim@kilindia.in</a>
              </div>
            </div>
          </aside>
        </div>
