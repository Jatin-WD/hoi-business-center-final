...
export default function ServiceDetailBySlug({ service, venues }: { service: CatalogService; venues: CatalogVenue[] }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-[#9a3412] to-[#f97316] text-white py-14 px-8">
        <div className="max-w-[1600px] mx-auto">
          <div className="flex items-center gap-2 text-orange-100 text-sm mb-4 flex-wrap">
      ...
      <div className="max-w-[1600px] mx-auto px-8 py-12 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
            {venues.map((venue) => (
              <Link key={venue.id} href={`/venue/${venue.locationId}/${venue.subVenueId}`} className="rounded-2xl border border-gray-100 bg-white p-5 transition-all hover:border-[#1a3a8f] hover:shadow-lg">
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-orange-50 text-[#f97316]">
                  <MapPin size={18} />
                </div>
        ...
        <div className="rounded-2xl bg-[#f97316] p-7 text-white">
          <h2 className="text-xl font-bold">Need this service for your exhibition?</h2>
          <p className="mt-2 text-orange-100">Send a requirement and the team will respond with pricing, availability, and next steps.</p>
